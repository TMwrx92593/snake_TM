...
def generuj_jablko():
    ...
    margin = 20 
    safe_zone_end_x = szerokość - margin  
    safe_zone_end_y = wysokość - margin

    jabłko_x = round(random.randrange(margin, (safe_zone_end_x // wielkosc_segmentu)) - 1) * wielkosc_segmentu
    jabłko_y = round(random.randrange(margin, (safe_zone_end_y // wielkosc_segmentu)) - 1) * wielkosc_segmentu
...
def pokaz_game_over(wynik):
    ...
    game_over_x  = (szerokość - game_over_text.get_width()) / 2
    game_over_y  = wysokość / 2 - 50
    wynik_x = (szerokość - wynik_text.get_width()) / 2
    wynik_y = wysokość / 2

    okno.blit(game_over_text, (game_over_x , game_over_y))
...
def gra():
    ...
    while not game_over:
        ...
        glowa = [x1, y1]
        segmenty_węża.append(glowa)
        if len(segmenty_węża) > dlugosc_węża:
            del segmenty_węża[0]

       

        for segment in segmenty_węża[:-1]:
            if segment == glowa:
                game_close = True

        rysuj_weza(segmenty_węża)
        pokaz_wynik(dlugosc_węża - 1)
        pygame.display.update()
